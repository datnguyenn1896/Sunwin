...
async def connect_and_communicate(uri, name):
    global previous_session, time, settings, bet, fund, result_next_seesion, total, total_win, total_lose, lose_strick, status_bet, gold
                        ...
                        if data[0] == 5 and data[1]["cmd"] == 1004:
                            ...
                            if result_next_seesion != None:
                                ...
                                if result_next_seesion == resultB:
                                    total_win += 1
                                    lose_strick = 0
                                    print(f"BET: {result_next_seesion} - Kết quả: {resultB} - WIN - Tổng số phiên: {total} - WIN: {total_win} - LOSE: {total_lose} - Tài khoản: {f'{gold:,}'}")
                                elif result_next_seesion != resultB:
                                    total_lose += 1
                                    lose_strick += 1
                                    print(f"BET: {result_next_seesion} - Kết quả: {resultB} - LOSE - Tổng số phiên: {total} - WIN: {total_win} - LOSE: {total_lose} - Tài khoản: {f'{gold:,}'}")
...
async def main():
    ...
    uri1 = "wss://websocket.azhkthg1.net/websocket"
    uri2 = "wss://websocket.azhkthg1.net/websocket4"

    # Tạo các tác vụ kết nối đến hai WebSocket server
    task1 = asyncio.create_task(connect_and_communicate(uri1, "websocket"))
